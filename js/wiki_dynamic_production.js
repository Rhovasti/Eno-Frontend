...
The technological disparity creates unique trade opportunities and cultural exchanges.`,
    ...
    async initializeMap() {
        ...
        try {
            ...
            this.basemapLayers = {
                satellite: L.tileLayer('https://rhovasti.github.io/eno-tiles/{z}/{x}/{y}.png', {
                ...
                relief: L.tileLayer('/relief-tiles/{z}/{x}/{y}.png', {
                    attribution: 'Eno World Map - Relief topography',
                    minZoom: 0,
                    maxZoom: 8,
                    opacity: 0.8,
                    tms: true  // Standard TMS format from gdal2tiles
                })
            };

            // Add default satellite basemap
            this.currentBasemap = 'satellite';
            this.basemapLayers.satellite.addTo(this.map);
    ...
    switchBasemap(basemapType) {
        ...
        if (this.basemapLayers[basemapType]) {
            ...
            console.log(`Switched to ${basemapType} basemap`);
        }
    }
    ...
    addGeoJSONToLayer(geojsonData, layerType) {
        ...
        if (layerType === 'cities' || layerType === 'villages') {
            this.locationData[layerType] = geojsonData.features.map(feature => ({
