...
app.post('/api/player-game-requests', authenticateToken, async (req, res) => {
    const { ai_gm_profile_id, game_title, genre, theme, max_players } = req.body;
...
Write an engaging description (max 150 words) that reflects the GM's personality style.`;
        ...
        db.run(
            `INSERT INTO games (title, description, created_by, genre, max_players, is_ai_gm, ai_gm_profile_id) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [game_title, gameDescription, 1, genre, max_players, 1, ai_gm_profile_id],
            function(err) {
                if (err) {
                    console.error('Error creating game:', err);
                    return res.status(500).json({ error: 'Failed to create game' });
                    ...
                    function(err) {
                        ...
                        db.run(
                            `INSERT INTO beats (chapter_id, title, content, sequence_number) 
                             VALUES (?, ?, ?, ?)`,
                            [chapterId, 'Alku', 'Tarinasi alkaa tästä. Kuvaile hahmosi ja toimintasi.', 1],
                            function(err) {
                                if (err) {
