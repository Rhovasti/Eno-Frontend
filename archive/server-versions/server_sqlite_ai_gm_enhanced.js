    ...
    db.get(
        ...
        async (err, user) => {
            ...
            try {
                ...
                const is_gm = roles.includes('gm');
                const is_admin = user.is_admin || roles.includes('admin');
...
app.post('/api/games', authenticateToken, requireGM, (req, res) => {
    ...
    db.run(
        `INSERT INTO games (name, description, genre, gm_id, max_players, post_frequency, is_private) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [title, description || '', genre || 'fantasy', req.user.id, 
...
Write in Finnish. Maximum 250 words. Be engaging and match your personality style.`;
                                        ...
                                        db.run(
                                            `INSERT INTO posts (beat_id, author_id, title, content, post_type) 
                                             VALUES (?, ?, ?, ?, ?)`,
                                            [beatId, 5, 'Pelin alku', introContent, 'gm'],
                                            (err) => {
                                        ...
                                        db.run(
                                            `INSERT INTO posts (beat_id, author_id, title, content, post_type) 
                                             VALUES (?, ?, ?, ?, ?)`,
                                            [beatId, 5, 'Pelin alku', fallbackIntro, 'gm'],
                                            (err) => {
...
Write in Finnish. Maximum 200 words.`;
                                    ...
                                    db.run(
                                        `INSERT INTO posts (beat_id, author_id, title, content, post_type) 
                                         VALUES (?, ?, ?, ?, ?)`,
                                        [beatId, 5, 'GM', responseContent, 'gm'],
                                        (err) => {
