...
app.post('/api/posts/:postId/generate-image', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const postQuery = 'SELECT user_id, beat_id FROM posts WHERE id = ?';
        db.query(postQuery, [postId], async (err, postResults) => {
            if (err) {
            ...
            try {
                ...
                if (sketch) {
                    ...
                    if (style === 'cartoon') {
                        // Use a random style image from the style folder
                    ...
                    // Use sketch-to-image with style transfer
                    if (styleImagePath) {
                        const fs = require('fs').promises;
                        const styleBuffer = await fs.readFile(styleImagePath);
                        // Use ControlNet with style reference
                        imageBuffer = await imageService.generateImageWithControlNet(
                            prompt + ', cartoon style, animated, stylized',
                            styleBuffer,
                            { 
                                style: 'anime',
                ...
                const imageKey = `posts/${postId}/image_${timestamp}.jpg`;
                const thumbnailKey = `posts/${postId}/thumbnail_${timestamp}.jpg`;
                ...
                // Save to database
                const insertQuery = `
                    INSERT INTO post_images (post_id, image_url, thumbnail_url, prompt, style, created_at)
                    VALUES (?, ?, ?, ?, ?, NOW())
                `;
                
                db.query(insertQuery, [postId, imageUrl, thumbnailUrl, prompt, style || null], (err, result) => {
                    if (err) {
